...
class GodMorgenSarpsborgTest(unittest.TestCase):
    ...
    def test_dagslys_skifter_retning_ved_solverv(self):
        self.assertIn("lengre", gms.beregn_dagslys_endring(datetime.date(2026, 6, 20)))
        self.assertIn("på sitt lengste", gms.beregn_dagslys_endring(datetime.date(2026, 6, 21)))
        self.assertIn("ca. 5 minutter kortere", gms.beregn_dagslys_endring(datetime.date(2026, 6, 30)))
        self.assertIn("kortere", gms.beregn_dagslys_endring(datetime.date(2026, 12, 20)))
        self.assertIn("på sitt korteste", gms.beregn_dagslys_endring(datetime.date(2026, 12, 21)))
        self.assertIn("lengre", gms.beregn_dagslys_endring(datetime.date(2026, 12, 22)))
